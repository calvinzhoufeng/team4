...
decl_storage! {
	...
	trait Store for Module<T: Trait> as TemplateModule {
		...
		ProofsWithMemo get(fn proofs_with_memo): map hasher(blake2_128_concat) Vec<u8> => (T::AccountId, T::BlockNumber, Vec<u8>);
	}
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn create_claim_with_memo(origin, claim: Vec<u8>, memo: Vec<u8>) -> dispatch::DispatchResult {
			...
			ensure!(!ProofsWithMemo::<T>::contains_key(&claim), Error::<T>::ProofAlreadyExist);

			ensure!(T::MaxClaimLength::get() >= claim.len() as u32, Error::<T>::ProofTooLong);
			ensure!(T::MaxClaimLength::get() >= memo.len() as u32, Error::<T>::MemoTooLong);

			ProofsWithMemo::<T>::insert(&claim, (sender.clone(), system::Module::<T>::block_number(), &memo));
