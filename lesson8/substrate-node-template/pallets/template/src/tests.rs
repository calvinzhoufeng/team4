...
fn test_onchain() {
	...
	t.execute_with(|| {
		...
		// added to storage
		assert_eq!(<Sum>::get(index), value);

		// an event is emitted
		let expected_event = TestEvent::template(RawEvent::NewSum(acct, index, value));
